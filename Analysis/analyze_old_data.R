...
# results <- lapply(c("../Data/old-data/", "../Data/old-data_er/", "../Data/old-data_cm/"), function(directory) {
    if (directory == "../Data/old-data/") randomization <- "None"
    if (directory == "../Data/old-data_er/") randomization <- "Erdos-Renyi"
    if (directory == "../Data/old-data_cm/") randomization <- "Configuration model"
...
#         my_file <- files[i]
        ...
        if (str_detect(my_file, "igraph")) {
            B <- get.incidence(g)
            metadata <- list(name = str_c(metadata$Subtype, "-", metadata$File),
                             type = "Mutualism",
                             feature_1 = metadata$Type,
                             feature_2 = NA,
                             feature_3 = NA,
                             feature_4 = NA,
                             nrows = metadata$N_Rows,
                             ncols = metadata$N_Cols,
                             connectance = metadata$N_Links / metadata$N_Rows / metadata$N_Cols,
                             n_links = metadata$N_Links)
        } else {
            B <- Data$B
            g <- from_incidence_matrix_to_bipartite(B)
            metadata <- list(name = Data$Name,
                             type = Data$Type,
                             feature_1 = Data$SubType,
                             feature_2 = str_c(Data$RowIdentity, "x", Data$ColIdentity),
                             feature_3 = NA,
                             feature_4 = NA,
                             nrows = Data$NRows,
                             ncols = Data$NCols,
                             connectance = Data$NLinks / Data$NRows / Data$NCols,
                             n_links = Data$NLinks)
        }
        ...
        if (is.na(lap_mat_eigs$values[1]) & nrow(lap_mat) < MAX_SIZE) lap_mat_eigs <- eigen(lap_mat)
        l1_lap <- lap_mat_eigs$values[1]
        l2_lap <- lap_mat_eigs$values[2]
        l3_lap <- lap_mat_eigs$values[3]
        alg_conn <- tryCatch(spectrum(lap_graph, which=list(pos="SA", howmany=2), options=list(maxiter=5000))$values[2],
                             error=function(e) {return(NA)})
        if (is.na(alg_conn) & length(lap_mat_eigs$values) == nrow(lap_mat)) alg_conn <- tail(lap_mat_eigs$values, 2)[1]
        ## some motif metrics
