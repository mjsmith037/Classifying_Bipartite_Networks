...
ggbiplot <- function(pcobj, choices = 1:2, scale = 1, pc.biplot = TRUE,
                     ...
                     groups = NULL, subgroups=NULL, ellipse = FALSE, ellipse.prob = 0.68,
    ...
    ## Error Catching
    if (length(choices) != 2)
        stop("Please choose two principal components to plot against one another")
    if (any(choices > ncol(u)))
        stop("Principal component choice greater than number of principal components")
    ...
    names(df.v) <- names(df.u) <- c('xvar', 'yvar')

    if (pc.biplot) df.u <- df.u * nobs.factor
    ...
    if (obs.scale == 0) {
        u.axis.labs <- paste0('standardized PC', choices)
    } else {
    ...
    if (length(alpha) == 1) alpha <- rep.int(alpha, nrow(df.u))

    if (var.axes) {
        # Draw circle
        if (circle) {
            theta <- c(seq(-pi, pi, length=50), seq(pi, -pi, length=50))
            circle_path <- data.frame(xvar=r*cos(theta), yvar=r*sin(theta))
            g <- g + geom_path(color=muted('white'), size=1/2, alpha=1/3, data=circle_path)
        }
    ...
    if (!is.null(df.u$labels)) {
        ...
        if (!is.null(df.u$groups)) {
            if (is.null(subgroups)) {
                ## if there are groups, but no subgroups, only use colour
                g <- g + geom_point(aes(color=groups, alpha=alpha))
            } else if (identical(groups, subgroups)) {
                ## if the same grouping should be used for colours and shapes
                g <- g +
                    geom_point(aes(color=groups, shape=groups, alpha=alpha))
            } else {
                ## if there are different colour and shape specifications
                g <- g +
                    geom_point(aes(color=groups, shape=subgroups, alpha=alpha))
            }
            ## if there are no groups, just plot all black points
        } else g <- g + geom_point(aes(alpha=alpha))
    }
    ...
    if (!is.null(df.u$groups) && ellipse) {
        ...
        ellipse_path <- function(x) {
            ...
            sigma <- x %>% select(-groups) %>% var()
            mu <- x %>% select(-groups) %>% summarise_all("mean") %>% as.numeric()
            ed <- ellipse.prob %>% qchisq(df=2) %>% sqrt()
        ...
        ell <- df.u %>%
            ...
            do(ellipse_path(.)) %>% 
            ungroup()
        g <- g + geom_path(aes(color=groups, group=groups), data=ell)
    }
    # Label the variable axes
    if (var.axes) {
        g <- g + geom_text(aes(label=varname, x=xvar, y=yvar, angle=angle, hjust=hjust),
                      color="darkred", size=varname.size, data=df.v)
    }
    ## suppress alpha in legend (does not work as expected)
